...
for CLUSTERED in [0,1]:
	...
	for did in dend_ids:
		...
		branch_hist = [];
		for run in range(NRUNS):
			spikes = np.loadtxt( './data/%s_%d_%d_%s_%d/spikesperpattern.dat'%(SOMCONDITION, dend_cond, CLUSTERED, prp, run), dtype=float)
			spikes = spikes[ 0:NPYRS]
			tr=  trevrolls(spikes)
			...
			syns = np.loadtxt('./data/%s_%d_%d_%s_%d/syn-post.txt'%(SOMCONDITION, dend_cond, CLUSTERED, prp, run), dtype=float)
			syns = syns[syns[:,4]>0.7]
			cols = ['input_id', 'group_id', 'branch_id','nid','weight']
			table = pd.DataFrame(syns,columns=cols)
			totals = table.groupby(['branch_id'])['weight'].count().values
...
plt.savefig("bp_%s.pdf"%(SOMCONDITION))
